...
//#define	CS_LOW()	PORTC.PODR.BYTE &= ~(0x10);		/* Set CS Low */
//#define	CS_HIGH()	PORTC.PODR.BYTE |= 0x10;		/* Set CS High */
...
#define FCLK_FAST() {					\
	...
	RSPI0.SPBR = F_PCLK/2/SCLK_FAST-1;	\
...
//	RSPI0.SPCR.BYTE = 0;		/* Stop RSPI module */
...
//	RSPI0.SPBR = F_PCLK / 2 / SCLK_SLOW - 1;	/* Bit rate */
	RSPI0.SPCR.BYTE = 0xE9;	/* Start RSPI in master mode */
}
...
)
{
	...
	RSPI0.SPDR.LONG = (BYTE)dat;			/* Start transmission (lower 8bits) */
	while (IR(RSPI0, SPRI0) == 0);  /* Wait for end of transfer */
	IR(RSPI0, SPRI0) = 0;
	return RSPI0.SPDR.LONG;			/* Returen received byte (lower 8bits) */
}
...
)
{
	...
	RSPI0.SPCMD0.BIT.SPB = 3;	/* Set 32-bit mode */

	do {
		while (RSPI0.SPSR.BIT.IDLNF) ;
		RSPI0.SPDR.LONG = LDDW(*lp++);	/* Send four data bytes */
		while (IR(RSPI0, SPRI0) == 0);  /* Wait for end of transfer */
		RSPI0.SPDR.LONG;					/* Discard four received bytes */
		IR(RSPI0, SPRI0) = 0;
	} while (btx -= 4);					/* Repeat until all data sent */

	RSPI0.SPCMD0.BIT.SPB = 7;	/* Set 8-bit mode */
}
...
)
{
	...
	do {
		while (RSPI0.SPSR.BIT.IDLNF) ;
		RSPI0.SPDR.LONG = 0xFFFFFFFF;	/* Send four 0xFFs */
		while (IR(RSPI0, SPRI0) == 0);  /* Wait for end of transfer */
