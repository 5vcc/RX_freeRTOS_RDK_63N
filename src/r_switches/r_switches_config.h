...
/* This macro sets whether interrupts or polling is used for detecting switch presses. The benefit of using interrupts
   ...
   1 = Use polling
    */
#define SWITCHES_CFG_DETECTION_MODE     (0)
...
/* The definition for these macros should be the name of the function that you want called when a switch is 
   ...
   This function's prototype should look like:
   void sw1_callback(void * pdata);

   When the callback is called, a pointer to a sw_callback_arg_t structure will be passed as the argument and cast 
   as (void *). Please see r_switches_if.h for more information on the sw_callback_arg_t structure. Please note that 
   this structure will only be valid during the callback function. If you need the contents of this structure after the
   callback then store this data in your application.

   Example: If SWITCHES_CFG_SW1_CALLBACK_FUNCTION is defined to be sw1_callback then the sw1_callback function will be 
   called when switch 1 is pressed.   

   If you do not wish to have a callback for a switch then comment out its macro below.
*/
#define SWITCHES_CFG_SW1_CALLBACK_FUNCTION       (sw1_callback)
#define SWITCHES_CFG_SW2_CALLBACK_FUNCTION       (sw2_callback)
#define SWITCHES_CFG_SW3_CALLBACK_FUNCTION       (sw3_callback)
